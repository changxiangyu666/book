...
<%
	...
	rs = stmt
			.executeQuery("select count(*) from orderform where orderid is not null "
					+ s_where);
	rs.next();
							...
							<table width="100%" border="0" cellpadding="1" cellspacing="1"
								...
								<tr>
									<td colspan="8">
										<div align="right">
											&nbsp;&nbsp;&nbsp;&nbsp;
											<a href="admin/ordersearch.jsp">查询</a>
										</div>
									</td>
								</tr>
								<tr>
								...
								<%
									rs = stmt
												.executeQuery("select * from orderform  where orderid is not null "
														+ s_where + " order by orderdate desc ");
										...
										for (int i = 0; i < i_pagenum; i++) {
											if (!rs.isLast()) {
												...
												s_orderdate = rs.getString("orderdate")
														.substring(0, 10);
												...
												Statement stmt2 = con.createStatement(
														...
														ResultSet.CONCUR_READ_ONLY);
												ResultSet rs2 = stmt2
														.executeQuery("select state from userinfo where userid='"
																+ s_userid + "' and state=0 ");
												if (rs2.next()) {
													s_state = "用户未激活";
												}
								%>
								<tr>
									<td>
										<a
											href="admin/orderlist.jsp?op=show&detailid=<%=s_orderid%>&currentpage=<%=i_currentpage%>"><%=s_orderid%></a>
									</td>
									<td><%=s_orderdate%></td>
									<td><%=s_receiver%></td>
									<td><%=s_address%></td>
									<td>
										<div align="center"><%=s_totalnum%></div>
									</td>
									<td>
										￥<%=s_totalamount%></td>
									<td><%=s_state%></td>
									<td>
										...
										%>
										<a
											href="admin/orderedit.jsp?orderid=<%=s_orderid%>&state=1&currentpage=<%=i_currentpage%>">发货</a>
										<%
											}
										%>
										<%
										...
										%>
										<a
											href="admin/orderedit.jsp?orderid=<%=s_orderid%>&state=2&currentpage=<%=i_currentpage%>">完毕</a>
										<%
											}
										%>
										<%
									...
									<td colspan="8">
										<div align="center">
											...
											<%
												if (i_currentpage > 1) {
											%>
											<a href="admin/orderlist.jsp?op=firstpage">首页</a>
											<%
												} else {
											%>首页<%
												}
											...
											%>
											<a
												href="admin/orderlist.jsp?op=previous&currentpage=<%=i_currentpage%>">上一页</a>
											<%
												} else {
											%>上一页<%
												}
											...
											%>
											<a
												href="admin/orderlist.jsp?op=next&currentpage=<%=i_currentpage%>">下一页</a>
											<%
												} else {
											%>下一页<%
												}
											...
											<%
												if (i_currentpage != i_totalpage) {
											%>
											<a href="admin/orderlist.jsp?op=lastpage">末页</a>
											<%
												} else {
											%>末页<%
												}
								...
								<tr>
									<td colspan="8">
										<table width="100%" border="0" cellpadding="1" cellspacing="1"
											...
											<%
												rs = stmt
															.executeQuery("select * from orderdetail where orderid='"
																	+ s_detailid + "' ");
													int i_total = 0;
